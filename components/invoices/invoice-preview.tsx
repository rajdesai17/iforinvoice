      ...
      <div className="p-4 bg-white text-[#111113] flex-1">
        <div className="space-y-3">
          ...
          <div className="flex justify-between items-start">
            <div>
              <h2 className="text-xs font-mono font-semibold text-primary tracking-wider uppercase">
                Invoice
              </h2>
              <p className="text-xl font-mono font-bold text-primary mt-0.5">{invoiceNumber}</p>
            </div>
            <div className="text-right">
              <p className="font-semibold text-sm text-[#111113]">{businessProfile?.businessName || "Your Business"}</p>
              {businessProfile?.email && (
                <p className="text-xs text-[#6b7280]">{businessProfile.email}</p>
              )}
              {businessProfile?.addressLine1 && (
                <p className="text-xs text-[#6b7280]">{businessProfile.addressLine1}</p>
              )}
            ...
            <div>
              ...
              {client ? (
                <>
                  ...
                  {client.company && <p className="text-xs text-[#6b7280]">{client.name}</p>}
                  {client.email && (
                    <p className="text-xs text-[#6b7280]">{client.email}</p>
                  )}
                </>
              ) : (
                <p className="text-[#9ca3af] italic text-xs">Select a client</p>
              )}
            </div>
            <div className="text-right">
          ...
          {/* Line Items Table */}
          <div className="overflow-hidden border border-[#e5e7eb] rounded">
            {/* Header Row - Violet Background */}
            <div className="grid grid-cols-12 gap-2 px-3 py-2 bg-primary text-white text-[10px] font-semibold uppercase tracking-wider">
            ...
            <div className="divide-y divide-[#e5e7eb]">
              {validLineItems.length > 0 ? (
                validLineItems.map((item) => (
                  <div key={item.id} className="grid grid-cols-12 gap-2 px-3 py-2 text-xs">
